...
const LgWeather = ({ post }) => {
  return (
    <section className='w-full h-auto p-5 max-sm:p-0'>
      <div className='lg_weather overflow-x-auto container mx-auto h-full flex gap-10 max-md:gap-5'>
        {post?.forecast?.forecastday.map((day, index) => (
          ...
          >
            ...
            <p className='text-white text-xl max-md:text-lg'>
              {day.day.condition.text}
            </p>
            <p className='text-gray-500 text-xl max-md:text-lg'>{day.date}</p>
            <p className='text-white text-6xl max-md:text-4xl'>
              {day.day.avgtemp_c}
            </p>
          </div>
        ))}
